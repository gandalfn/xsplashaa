...
int detector::runDetector(IplImage* input)
{
  ...
  {
    if(flag != 1 || lengthEye_ == 0) messageIndex = 2; // Detected the face, but not the eyes
  }

  if(flag == 1 && lengthEye_ != 0)
  {
    newWidth = floor( (prevlengthEye_ * widthEyeWindow_) / lengthEye_);
    newHeight =  floor( (prevlengthEye_ * heightEyeWindow_) / lengthEye_);
  }

  if(flag == 1 && lengthEye_ > 0 && newWidth > 0 && newHeight > 0 && prevlengthEye_ > 0)
  {
    ...
    if( (lx + newWidth) > FA_IMAGE_WIDTH) newWidthL = FA_IMAGE_WIDTH - lx;
    if( (ly +newHeight) > FA_IMAGE_HEIGHT) newHeightL = FA_IMAGE_HEIGHT - ly;
    ...
    if( (rx+newWidth) > FA_IMAGE_WIDTH) newWidthR = FA_IMAGE_WIDTH - rx;
    if( (ry+newHeight) > FA_IMAGE_HEIGHT) newHeightR = FA_IMAGE_HEIGHT - ry;
