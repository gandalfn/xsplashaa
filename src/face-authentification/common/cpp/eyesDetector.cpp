...
const string HAAR_CASCADE_EYE = PACKAGE_FACEAUTH_DATA_DIR "/haarcascade_eye_tree_eyeglasses.xml";
const string HAAR_CASCADE_EYE_2 = PACKAGE_FACEAUTH_DATA_DIR "/haarcascade_eye.xml";
...
void eyesDetector::runEyesDetector(IplImage* input, IplImage* fullImage, CvPoint LT)
{
    ...
    if(count > 0)
    {
        for(int j = 0; j < (nested_objects ? nested_objects->total : 0); j++)
        {
            ...
            if((center.x - 4) > 0 && ((center.x - 4) < (FA_IMAGE_WIDTH-8)) &&
               (center.y - 4) > 0 && ((center.y - 4) < (FA_IMAGE_HEIGHT-8)))
            {
