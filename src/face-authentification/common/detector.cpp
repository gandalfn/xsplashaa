...
const char * detector::queryMessage()
{
    std::string message0 ("Please come closer to the camera.");
    std::string message1 ("Please go little far from the camera.");
    std::string message2 ("Unable to Detect Your Face.");
    std::string message3 ("Tracker lost, trying to reinitialize.");
    std::string message4 ("Tracking in progress.");
    std::string message6 ("Capturing Image Finished.");

    if (messageIndex==-1)
        return 0;
    else if (messageIndex==0)
        return message0.c_str ();
    else if (messageIndex==1)
        return message1.c_str ();
    else if (messageIndex==2)
        return message2.c_str ();
    else if (messageIndex==3)
        return message3.c_str ();
    else if (messageIndex==4)
        return message4.c_str ();
    else if (messageIndex==5)
        return messageCaptureMessage;
    else if (messageIndex==6)
        return message6.c_str ();
    return 0;
